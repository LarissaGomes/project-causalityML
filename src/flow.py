...
class GraphExplainer:
    ...
    def set_noise_sampler(self):
        ...
        for node in topo_sort(self.graph):
            ...
            if len(node.args) > 0:
                print("node args > 0")
                print([arg.bg_val for arg in node.args])
                print(node.f)
                ...
                if node.name in bg_values:
                    ...
                    if bg_sampled.shape != bg_computed.shape or \
                       (bg_sampled != bg_computed).any():
                        print("bg sampled shape != bg computed shape or bg sampled != bg computed")
                        ...
                        if bg_sampled.shape != bg_computed.shape:
                            ...
                            print("bg sampled shape != bg computed shape")
                    ...
                    print("node name not in bg values")
                    node.bg_val = bg_computed
                    print("node bg val = bg computed")
                    node.fg_val = fg_computed
                    print("node fg val = fg computed")
    ...
    def shap_values(self, X, method='bruteforce_sampling',
        ...
        # if not skip_prepare:
        #     try:
                # print("preparing graph:",X)
        self.prepare_graph(X)
        print("passou prepare graph")
